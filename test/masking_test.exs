defmodule EndPointBlank.MaskingTest do
  ...
  describe "reference vectors" do
    test "path only: $.user.ssn" do
      ...
      out = Masking.apply(payload, :request, [rule("request_body", path: "$.user.ssn", replacement_value: "***")], nil)
    ...
    test "path only recursive descent: $..password" do
      ...
      out = Masking.apply(payload, :request, [rule("request_body", path: "$..password", replacement_value: "***")], nil)
      assert Jason.decode!(out.request) == %{"a" => %{"password" => "***"}, "b" => %{"password" => "***"}}
    end

    test "path + regex: $.note with SSN regex" do
      payload = %{request: ~s({"note":"ssn 123-45-6789"})}
      r = rule("request_body", path: "$.note", regex: "\\d{3}-\\d{2}-\\d{4}", replacement_value: "XXX")
    ...
    test "path with wildcard over a list: $.list[*].k" do
      ...
      out = Masking.apply(payload, :request, [rule("request_body", path: "$.list[*].k", replacement_value: "_")], nil)
    ...
    test "path no-ops on a non-JSON plain string target" do
      ...
      out = Masking.apply(payload, :request, [rule("path", path: "$.x", replacement_value: "_")], nil)
  ...
  describe "headers (map target)" do
    test "path selects a header key and replaces it" do
      ...
      out = Masking.apply(payload, :request, [rule("request_headers", path: "$.Authorization")], nil)
  ...
  describe "graceful degradation" do
    test "non-JSON body: path no-ops, regex still applies to raw string" do
      ...
      r = rule("request_body", path: "$.email", regex: "[\\w.]+@[\\w.]+", replacement_value: "...")
      out = Masking.apply(payload, :request, [r], nil)
    ...
    test "bad regex degrades to no-op" do
      ...
      out = Masking.apply(payload, :request, [rule("path", regex: "([unterminated", replacement_value: "_")], nil)
      assert out.path == "/users/x"
    end

    test "malformed path degrades to no-op" do
      payload = %{request: ~s({"a":"b"})}
      out = Masking.apply(payload, :request, [rule("request_body", path: "not a path", replacement_value: "_")], nil)
    ...
    test "blank replacement_value coerces to ..." do
      payload = %{request: ~s({"a":"b"})}
      out = Masking.apply(payload, :request, [rule("request_body", path: "$.a", replacement_value: "")], nil)
